...
export function useO2TaxMetas(startDate?: Date, endDate?: Date) {
  const { data, isLoading, error, refetch } = useQuery({
    ...
    queryFn: async (): Promise<O2TaxMetasResult> => {
      ...
      console.log(`[useO2TaxMetas] Unique fases atuais:`, uniqueFasesAtuais);
    ...
    for (const movement of data.movements) {
      ...
      if (entryTime >= startTime && entryTime <= endTime) {
        ...
        if (indicator === 'leads') {
          ...
          // For "venda", count unique cards that ENTERED "Ganho" or "Contrato assinado" phase during the period
          if (movement.fase === 'Ganho' || movement.fase === 'Contrato assinado') {
            uniqueCards.add(movement.id);
          }

...
export type TabKey = 'context' | 'goals' | 'monthly' | 'media' | 'marketing' | 'structure' | 'admin' | 'indicators' | 'marketing_indicators';
...
export function useUserPermissions(userId: string | undefined) {
  ...
  const allowedTabs: TabKey[] = isAdmin 
    ? ['context', 'goals', 'monthly', 'media', 'marketing', 'structure', 'admin', 'indicators', 'marketing_indicators']
    : permissions || [];

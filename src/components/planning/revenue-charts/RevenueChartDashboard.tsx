// Option 5: Compact Dashboard - KPI cards with sparklines + consolidated chart
import { useMemo } from "react";
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";
import { ComposedChart, Bar, Line, XAxis, YAxis, CartesianGrid, ResponsiveContainer, Tooltip, Legend, AreaChart, Area } from "recharts";
import { LayoutDashboard, TrendingUp, TrendingDown } from "lucide-react";
import { BUType } from "@/hooks/useFunnelRealized";
import { Progress } from "@/components/ui/progress";

interface RevenueChartDashboardProps {
  startDate: Date;
  endDate: Date;
  selectedBUs: BUType[];
  chartData: {
...
export function RevenueChartDashboard({ 
  startDate, 
  endDate, 
  selectedBUs,
  chartData,
  ...
  return (
    <Card className="bg-card border-border">
      ...
      <CardContent className="pt-2 space-y-4">
        ...
        {/* Consolidated Stacked Bar Chart */}
        <div className="h-64">
          <ResponsiveContainer width="100%" height="100%">
            <ComposedChart data={chartData} margin={{ top: 10, right: 10, left: 0, bottom: 5 }}>
              ...
              {selectedBUs.includes('modelo_atual') && (
                <Bar dataKey="modelo_atual" stackId="revenue" fill={BU_COLORS.modelo_atual} name="modelo_atual" radius={[0, 0, 0, 0]} />
              )}
              {selectedBUs.includes('o2_tax') && (
                <Bar dataKey="o2_tax" stackId="revenue" fill={BU_COLORS.o2_tax} name="o2_tax" radius={[0, 0, 0, 0]} />
              )}
              {selectedBUs.includes('oxy_hacker') && (
                <Bar dataKey="oxy_hacker" stackId="revenue" fill={BU_COLORS.oxy_hacker} name="oxy_hacker" radius={[0, 0, 0, 0]} />
              )}
              {selectedBUs.includes('franquia') && (
                <Bar dataKey="franquia" stackId="revenue" fill={BU_COLORS.franquia} name="franquia" radius={[4, 4, 0, 0]} />
              )}
